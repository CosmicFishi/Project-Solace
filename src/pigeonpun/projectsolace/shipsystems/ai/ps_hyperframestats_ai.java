...
public class ps_hyperframestats_ai implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        //if pursuing a target and target is over 80% flux, kill them
        if(ship.getShipTarget() != null && flags.hasFlag(ShipwideAIFlags.AIFlags.PURSUING) && ship.getShipTarget().getFluxLevel() > 0.8f) {
            useSystem = true;
        }
        //if move in to attack and target close enough
        if(flags.hasFlag(ShipwideAIFlags.AIFlags.MANEUVER_TARGET) && isCloseToTarget) {
            useSystem = true;
        }
        //Move out when flux goes up
